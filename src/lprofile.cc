...
namespace Lcurve {
    int run_lprofile(const std::string& model_file){
        ...
        // This forces to use the model_file
        const char* argv[] = {"lprofile", model_file.c_str()};
        int argc = 2;
        ...
        try{
            ...
            for(int np=0; np<nphase; np++){
                ...
                std::cout << "Phase: " << form(phase) << "  Vmean: " << form(vwmean/wmean) << std::endl;
                // blurr
